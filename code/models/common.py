...
class convMHSAa(nn.Module):
    ...
    def forward(self, x):
        ...
        # w3 = self.w2
        # w4 = self.w3
        weight1 = w1 / (torch.sum(w1, dim=0) + self.epsilon)  # 将权重进行归一化
        weight2 = w2 / (torch.sum(w2, dim=0) + self.epsilon)  # 将权重进行归一化
        # weight3 = w3 / (torch.sum(w3, dim=0) + self.epsilon)  # 将权重进行归一化
        # weight4 = w4 / (torch.sum(w4, dim=0) + self.epsilon)  # 将权重进行归一化
        m_batchsize, C, width, height = x.size()
