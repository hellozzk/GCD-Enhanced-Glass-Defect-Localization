...
class GCM(nn.Module):
    def __init__(self, in_channels1, in_channels2, out_channels1, reduction=8):  # (256, 128, 128)
        super(GCM, self).__init__()
        self.in1 = in_channels1
        self.in2 = in_channels2
        # self.ou1 = out_channels1
        self.Z_conv = nn.Conv2d(in_channels1*2, out_channels1 // reduction, 1)
        self.P_conv = nn.Conv2d(in_channels1*2, out_channels1 // reduction, 1)
        self.value_conv = nn.Conv2d(in_channels2, in_channels2, 1)
        self.sigmoid = nn.Sigmoid()
        self.avg = nn.AdaptiveAvgPool2d(2)
        self.down = down(in_channels1)
        self.cv1=nn.AvgPool2d(2)
        self.bn = nn.BatchNorm2d(in_channels1)
        self.MLP=FeedForward(in_channels1*2)
        self.bn1 = nn.BatchNorm2d(in_channels1*2)
    def forward(self, x):
        F = x[1]
        F5 = x[0]
        F=self.bn(F)
        ...
        out = self.MLP(self.bn1((self.down(proj_value * attention_S))))  # B x W x H
        return out
...
class GCM1(nn.Module):
    def __init__(self, in_channels1, in_channels2, out_channels1, reduction=8):  # (256, 128, 128)
        ...
        self.Z_conv = nn.Conv2d(in_channels1*2, out_channels1 // reduction, 1)
        self.P_conv = nn.Conv2d(in_channels1*2, out_channels1 // reduction, 1)
        self.value_conv = nn.Conv2d(in_channels2//2, in_channels2, 1)
        self.sigmoid = nn.Sigmoid()
        self.avg = nn.AdaptiveAvgPool2d(2)
        self.down = down(in_channels1*2)
        self.cv1=nn.AvgPool2d(2)
        self.bn = nn.BatchNorm2d(in_channels1)
        self.MLP=FeedForward(in_channels1*4)
        self.bn1 = nn.BatchNorm2d(in_channels1*2)
        self.bn2 = nn.BatchNorm2d(in_channels1 * 4)
    def forward(self, x):
        F = x[0]
        F5 = x[1]
        # print(F.size(), F5.size(), self.in1,self.in2)
        F=self.bn(F)
        F5 = self.bn1(F5)
        m_batchsize, C, width, height = F5.size()
        proj_query = self.P_conv(F5).view(m_batchsize, -1, width * height).permute(0, 2, 1)  # B x N x C''
        ...
        attention_S=attention_S
        proj_value = self.value_conv(F)
        # print('1111',attention_S.size())
        # print((self.down(proj_value * attention_S)).size())
        out = self.cv1(self.MLP(self.bn2((self.down(proj_value * attention_S)))))  # B x W x H
        # print(out.size())
        return out
...
class GCM2(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_S = self.sigmoid((self.avg(S).reshape(m_batchsize, -1, width * 2, height * 2)))  # B x 1 x W x H
        # print(attention_S.size())
        attention_S=self.cv1(attention_S)
        proj_value = self.value_conv(F)
        # print('1111',proj_value.size())
        # print((self.down(proj_value * attention_S)).size())
        out = self.MLP(self.bn2((self.down(proj_value * attention_S))))  # B x W x H
        # print(out.size())
        return out
# *************======================================================================*************
# *************======================================================================*************
...
# *************======================================================================*************
# *************======================================================================*************
class Score_aggregator(nn.Module):

    def __init__(self, channel, reduction=16):
        super(Score_aggregator, self).__init__()
    ...
    def forward(self, x):
        b, c, _, _ = x.size()
        y = self.avg_pool(x).view(b, c)#Select high-frequency feature
...
class EdgeGuidedBlock(nn.Module):
    def __init__(self, nIn, nOut, dilation_rate=2, reduction=16, add=True):
        ...
        self.add = add
        self.F_glo = Score_aggregator(nOut, reduction)
...
# class EdgeGuidedBlock_Down(nn.Module):
#     """
...
class EGM(C3):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.Sequential(*(EdgeGuidedBlock(c_, c_) for _ in range(n)))
# *************======================================================================*************
# *************======================================================================*************
# *************======================================================================*************
# *************======================================================================*************
...
class Concat_b(nn.Module):
    ...
    def __init__(self, dimension=1,c1=0):
        ...
        self.d = dimension
        self.conv=Conv(c1*2,c1, 1, 1)
        self.conv1 = Conv(c1*2, c1, 1, 1)
        # self.bn = nn.BatchNorm2d(c1)
        self.c1 = c1
    def forward(self, x):
        # print('ConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCat')
        ...
        x1=self.conv1(x[1])
        # print(x1.size())
        out=self.conv(torch.cat((x[0],x1), self.d))
        # print(out.size())
...
class FIF(nn.Module):
    ...
    def __init__(self, dimension=1,c1=0):
        ...
        self.conv1 = Conv(c1*2, c1, 1, 1)
        # self.bn = nn.BatchNorm2d(c1)
        self.GCM1=GCM1(in_channels1, in_channels2, out_channels1,8)
        self.c1 = c1
    def forward(self, x):
        # print('ConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCatConCat')
        # print(x[0].size(),x[1].size())
